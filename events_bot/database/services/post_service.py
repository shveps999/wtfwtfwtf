...
class PostService:
    ...
    @staticmethod
    def _parse_and_prepare_datetime(event_at_iso: str | None) -> datetime | None:
        """
        Вспомогательный метод для парсинга строки ISO 8601 и преобразования в UTC naive datetime.
        Это обеспечивает консистентность хранения времени в PostgreSQL и SQLite.
        """
        if event_at_iso is None:
            return None
        try:
            parsed_dt = datetime.fromisoformat(event_at_iso)
            if parsed_dt.tzinfo is not None:
                parsed_dt = parsed_dt.astimezone(timezone.utc)
            return parsed_dt.replace(tzinfo=None)
        except (ValueError, TypeError) as e:
            logfire.error(f"Не удалось распарсить дату: '{event_at_iso}'. Ошибка: {e}")
            return None
    ...
    async def create_post(
        ...
        """Создать новый пост"""
        parsed_event_at = PostService._parse_and_prepare_datetime(event_at)
    ...
    async def send_post_to_moderation(bot, post: Post, db=None):
        ...
        if not moderation_group_id_str:
            logfire.error("MODERATION_GROUP_ID не установлен, пост не может быть отправлен на модерацию.")
            return
        try:
            moderation_group_id = int(moderation_group_id_str)
        except (ValueError, TypeError):
            logfire.error(f"MODERATION_GROUP_ID имеет неверное значение: {moderation_group_id_str}")
            return
        if db:
        ...
        logfire.info(f"Отправка поста {post.id} на модерацию в группу {moderation_group_id}")
        try:
            if post.image_id:
                ...
                if media_photo:
                    ...
                    logfire.warning(f"Изображение для поста {post.id} не найдено, отправка текстом.")
            ...
            logfire.info(f"Пост {post.id} успешно отправлен на модерацию.")
        except Exception as e:
            import traceback
            logfire.error(f"Ошибка при отправке поста {post.id} на модерацию: {e}")
