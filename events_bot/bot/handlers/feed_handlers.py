...
# ZoneInfo fallback - rapordaki sorun
try:
    from zoneinfo import ZoneInfo
except ImportError:
    # Python < 3.9 or zoneinfo not available, use pytz fallback or timezone
    try:
        import pytz
        ZoneInfo = lambda name: pytz.timezone(name)
    except ImportError:
        # Neither zoneinfo nor pytz available, use simple timezone
        ZoneInfo = None
...
async def show_feed_page_cmd(message: Message, page: int, db):
    ...
    # Список кратких карточек - db.refresh kaldırıldı (performans için)
...
async def show_feed_page(callback: CallbackQuery, page: int, db):
    ...
    total_pages = (total_posts + POSTS_PER_PAGE - 1) // POSTS_PER_PAGE
    # db.refresh kaldırıldı (performans için)
    preview_text = format_feed_list(posts, page * POSTS_PER_PAGE + 1, total_posts)
    try:
        await callback.message.edit_text(
            preview_text,
...
def _msk_str(dt) -> str:
    """Timezone conversion with fallback"""
    if not dt:
        return ""
    ...
    try:
        if ZoneInfo:
            ...
            # Fallback: простое смещение +3 часа для МСК
            from datetime import timedelta
            dt = dt + timedelta(hours=3)
    except Exception:
        # В случае любой ошибки показываем время как есть
        pass
...
async def show_post_details(callback: CallbackQuery, post_id: int, current_page: int, total_pages: int, db):
    """Показать детали поста с обработкой ошибок file_storage"""
    post = await PostService.get_post_by_id(db, post_id)
    if not post:
        await callback.answer("Пост не найден", show_alert=True)
        return
    
    # db.refresh убран (используем selectinload в PostService)
    ...
    # file_storage.get_media_photo hata yönetimi eklendi
    if post.image_id:
        try:
            ...
            # file_storage hatası durumunda text-only göster
            logfire.error(f"Ошибка загрузки медиа для поста {post_id}: {e}")
    
    # Text-only post veya media hatası durumu
    try:
        await callback.message.edit_text(
            text,
            reply_markup=get_feed_post_keyboard(current_page, total_pages, post.id, is_liked, likes_count),
...
async def show_liked_post_details(callback: CallbackQuery, post_id: int, current_page: int, total_pages: int, db):
    """Показать детали избранного поста с обработкой ошибок"""
    post = await PostService.get_post_by_id(db, post_id)
    if not post:
        await callback.answer("Пост не найден", show_alert=True)
        return
    ...
    # file_storage.get_media_photo hata yönetimi
    if post.image_id:
        try:
            media_photo = await file_storage.get_media_photo(post.image_id)
            if media_photo:
            ...
            logfire.error(f"Ошибка загрузки медиа для избранного поста {post_id}: {e}")
